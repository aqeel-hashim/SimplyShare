...
import static data.musta.it.apiit.com.repository.connection.transfer.FileTransferService.ActualFilelength;
import static data.musta.it.apiit.com.repository.connection.transfer.FileTransferService.listner;
...
public class WifiP2pTransferManager implements TransferManager {
    ...
    public void send(Item item, TransferProgressListener listener) {
        ...
        ActualFilelength = Integer.parseInt(itemEntity.getSize());
        ...
        if (!TextUtils.isEmpty(OwnerIp) && OwnerIp.length() > 0) {
            ...
            serviceIntent.putExtra(FileTransferService.EXTRAS_FILE_TRANSFER_LISTNER, new FileTransferService.FileProgressListner() {
                @Override
